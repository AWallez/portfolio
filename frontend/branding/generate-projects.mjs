// Generates the 10 Projects-section visuals (5 cards × light/dark theme).
// Each card is recolored from the site tokens (src/index.css) and rendered twice
// (light / dark palette), so the images share exactly the site's colors.
// Output is SVG (vector: ~10× lighter than PNG, crisp at any zoom). The SVGs are
// inlined in Projects.tsx (not loaded via <img>), so they inherit the site's own
// webfonts — JetBrains Mono (mono) and Inter (sans) — and render identically for
// every visitor. IDs are made unique per file to avoid collisions between the
// inlined SVGs sharing the same document.
//
//   node branding/generate-projects.mjs   -> 10 SVG into src/assets/projects/
//
...
const LIGHT = {
  base: "#eef3f2",
  surface: "#ffffff",
  soft: "#f4f8f7", // inner panels / fields background
  ink: "#0a2229",
  muted: "#486169",
  line: "#c8d6d2",
  accent: "#0f766e",
  accentInk: "#ffffff",
  chrome: "#0a2229", // titlebar / dark device
  chromeInk: "#eef3f2",
  shadow: "#0a2229",
  shadowOp: 0.16,
  dotOp: 0.7,
...
/* Helpers                                                             */
/* ------------------------------------------------------------------ */
// Polices du site (chargées en webfonts) : les SVG étant inlinés, ils en héritent.
// Fallbacks génériques au cas où une webfont ne serait pas encore prête.
...
// three window dots (declined in the accent palette)
...
// dark titlebar (terminal/code window)
...
// tag pills (like the site's <Tag>: accent 12% bg, accent 32% border, mono)
...
/* ================================================================== */
/* 1. clients (web) — 5 frames: site, dashboard, mobile, bookings, form */
/* ================================================================== */
function clients(p) {
  const a = p.accent;
  let s = "";
  ...
  s += T(48, 56, 13, p.muted, "Showcase site — craftsperson / freelancer");
  s += card(48, 66, 624, 300, p, { r: 12 });
  s += dots3(68, 83, p);
  s += `<rect x="118" y="75" width="220" height="16" rx="8" fill="${p.soft}"/>${T(130, 87, 10, p.muted, "dupont-carpentry.com", { mono: true })}`;
  s += `<line x1="48" y1="100" x2="672" y2="100" stroke="${p.line}"/>`;
  s += `<rect x="66" y="116" width="14" height="14" rx="3" fill="${a}"/>${T(86, 128, 13, p.ink, "Dupont Carpentry", { w: "bold" })}`;
  s += T(386, 128, 11, p.muted, "Home");
  s += T(442, 128, 11, p.muted, "Projects");
  s += T(520, 128, 11, p.muted, "Services");
  s += `<rect x="584" y="116" width="70" height="22" rx="11" fill="${a}"/>${T(619, 131, 10, p.accentInk, "Quote", { w: "bold", anchor: "middle" })}`;
  s += T(66, 186, 24, p.ink, "The woodworker,", { w: "bold" });
  s += T(66, 216, 24, a, "made to measure.", { w: "bold" });
  s += T(66, 242, 12, p.muted, "Custom furniture design &amp; renovation");
  s += T(66, 259, 12, p.muted, "for homes &amp; businesses.");
  s += `<rect x="66" y="276" width="168" height="38" rx="9" fill="${a}"/>${T(150, 300, 12.5, p.accentInk, "Request a quote", { w: "bold", anchor: "middle" })}`;
  ...
  s += T(66, 342, 11, a, "▸ Fittings", { op: 0.85 });
  s += T(206, 342, 11, a, "▸ Custom furniture", { op: 0.85 });
  s += T(392, 342, 11, a, "▸ Renovation", { op: 0.85 });

  /* ---- Frame 2: custom web app (StockApp) ---- */
  s += T(48, 400, 13, p.muted, "Custom web app — management");
  ...
  s += `<rect x="58" y="458" width="100" height="26" rx="6" fill="${a}" fill-opacity="0.14"/>${T(70, 475, 10.5, a, "Dashboard", { w: "bold" })}`;
  ["Customers (CRM)", "Inventory", "Bookings", "Analytics"].forEach((m, i) =>
  ...
  s += T(745, 118, 11, p.ink, "Dupont Carpentry", { w: "bold" });
  s += `<rect x="716" y="134" width="158" height="104" rx="10" fill="url(#thumb)"/><circle cx="752" cy="168" r="9" fill="${a}" fill-opacity="0.4"/>`;
  s += T(716, 268, 15, p.ink, "The custom", { w: "bold" });
  s += T(716, 288, 15, a, "woodworker", { w: "bold" });
  s += T(716, 310, 10, p.muted, "Furniture &amp; fittings");
  s += `<rect x="716" y="328" width="158" height="36" rx="9" fill="${a}"/>${T(795, 351, 12, p.accentInk, "Request a quote", { w: "bold", anchor: "middle" })}`;
  ...
  s += T(912, 56, 13, p.muted, "Bookings module");
  s += card(912, 66, 240, 360, p, { r: 14 });
  s += T(930, 102, 14, p.ink, "Bookings", { w: "bold" });
  s += T(1136, 102, 10, p.muted, "June 2026", { anchor: "end" });
  const resa = [
  ...
  s += `<rect x="928" y="376" width="208" height="38" rx="9" fill="${a}"/>${T(1032, 400, 12.5, p.accentInk, "+ New booking", { w: "bold", anchor: "middle" })}`;

  /* ---- Frame 5: contact form ---- */
  s += T(700, 456, 13, p.muted, "Contact form");
  s += card(700, 466, 452, 172, p, { r: 12 });
  s += T(718, 494, 13, p.ink, "Contact us", { w: "bold" });
  s += field(718, 506, 205, 30, p, "Name");
  s += field(935, 506, 205, 30, p, "Email");
  s += field(718, 544, 422, 40, p, "Your message…");
  s += `<rect x="718" y="594" width="422" height="32" rx="8" fill="${a}"/>${T(929, 615, 12, p.accentInk, "Send", { w: "bold", anchor: "middle" })}`;

  return frame(p, s);
}

/* ================================================================== */
/* 2. portfolio — desktop + mobile + GitHub + quality + ntfy flow       */
/* ================================================================== */
function portfolio(p) {
  ...
  s += T(70, 344, 11.5, p.muted, "Accessibility · SEO · i18n FR/EN · tests · CI/CD", { op: 0.85 });
  s += T(70, 368, 11.5, p.muted, "Full-stack, from front-end to deployment — DevOps-minded.");
  ...
  s += `<circle cx="824" cy="172" r="4" fill="${a}"/>${T(836, 176, 12, a, "Public · CI passing")}`;
  s += T(824, 200, 11, p.muted, "React · TS · Fastify · PostgreSQL · Docker");

  /* ---- Quality (tags) ---- */
  s += card(800, 234, 352, 154, p, { r: 12 });
  s += T(820, 262, 13, p.ink, "Polished", { w: "bold" });
  s += tagRow(820, 276, ["Accessibility", "SEO", "i18n FR/EN"], p);
  s += tagRow(820, 308, ["Responsive", "Vitest tests", "CI/CD Actions"], p);
  s += tagRow(820, 340, ["Light / dark theme", "Containerized"], p);

  /* ---- Flow: contact form → API → ntfy ---- */
  s += T(48, 412, 13, p.muted, "Contact form → API → push notification (ntfy)");
  s += card(48, 424, 372, 214, p, { r: 12 });
  s += T(66, 452, 13, p.ink, "Contact us", { w: "bold" });
  s += field(66, 462, 166, 28, p, "First name");
  s += field(240, 462, 164, 28, p, "Last name");
  s += field(66, 498, 166, 28, p, "Email");
  s += field(240, 498, 164, 28, p, "Phone");
  s += field(66, 534, 338, 28, p, "Request type");
  s += T(390, 552, 12, p.muted, "▾", { anchor: "middle" }); // select chevron
  s += field(66, 570, 338, 22, p, "Message…");
  s += `<rect x="66" y="600" width="338" height="26" rx="8" fill="${a}"/>${T(235, 618, 11, p.accentInk, "→ Send", { w: "bold", anchor: "middle" })}`;
  ...
  s += T(684, 500, 10, p.muted, "publish", { anchor: "middle" });

  s += T(718, 452, 13, p.muted, "Push notification");
  ...
  s += T(790, 515, 10.5, p.muted, "on your phone");
  s += T(1134, 498, 10, p.muted, "now", { anchor: "end", op: 0.8 });
  s += `<line x1="738" y1="536" x2="1132" y2="536" stroke="${p.line}"/>`;
  s += T(738, 560, 13, p.ink, "New message — Marie Dupont", { w: "bold" });
  s += T(738, 580, 11, p.muted, "marie@example.com · Freelance project");
  s += T(738, 600, 11, p.muted, "&quot;Hi, I'd like a quote to redesign my website…&quot;", { op: 0.85 });

  return frame(p, s);
}

/* ================================================================== */
/* 3. reseau — VLAN segmentation & security                            */
/* ================================================================== */
function reseau(p) {
  const a = p.accent;
  let s = "";
  ...
  s += T(566, 200, 14, p.ink, "Router / Firewall", { w: "bold" });
  ...
  s += T(1028, 126, 12.5, p.ink, "Remote access", { w: "bold" });
  s += T(1028, 143, 10, p.muted, "Encrypted VPN", { mono: true });
  ...
  s += T(620, 322, 14, p.chromeInk, "Managed switch · 10 GbE", { w: "bold", anchor: "middle" });
  ...
  s += vlan(125, "VLAN 10", "Servers", "NAS · Postgres · ntfy", "games · storage");
  s += vlan(375, "VLAN 20", "Workstations", "PC / dev", "work machines");
  s += vlan(625, "VLAN 30", "IoT / Smart home", "connected devices", "isolated from LAN");
  s += vlan(875, "VLAN 40", "Guests / Wi-Fi", "limited access", "no internal access");

  return frame(p, s);
}

/* ================================================================== */
/* 4. homelab (nas) — Linux NAS / Docker + services                    */
/* ================================================================== */
function homelab(p) {
  const a = p.accent;
  let s = "";
  ...
  s += T(138, 332, 13, p.ink, "Router", { w: "bold" });
  s += T(138, 350, 11, p.muted, "firewall", { mono: true });
  ...
  s += T(400, 190, 12, p.muted, "Containerized services", { w: "bold" });
  ...
  s += T(400, 448, 11, p.muted, "+ docker-compose · Linux · hardening · monitoring", { op: 0.85 });
  s += `<rect x="400" y="462" width="402" height="64" rx="10" fill="${p.soft}"/>`;
  s += T(416, 486, 11, p.muted, "$ docker compose up -d", { mono: true });
  s += `<circle cx="420" cy="504" r="4" fill="${a}"/>${T(432, 508, 11, a, "6 services running", { mono: true })}`;
  ...
  s += T(978, 504, 12.5, p.ink, "Remote access", { w: "bold" });
  s += T(978, 521, 10, p.muted, "encrypted / secured", { mono: true });

  return frame(p, s);
}

/* ================================================================== */
/* 5. lab (perso) — DevOps terminal                                    */
/* ================================================================== */
function lab(p) {
  const a = p.accent;
  let s = "";
  ...
  // columns (x) to align the mono tables
  ...
  // last prompt (no blinking cursor)
...
// rend uniques les ids partagés (dots/sh/thumb/arrow) pour que plusieurs SVG
// inlinés dans le même document ne se volent pas leurs gradients/filtres.
...
for (const [name, build] of Object.entries(BUILDERS)) {
  for (const [theme, palette] of [
    ...
    writeFileSync(`src/assets/projects/${uid}.svg`, uniquify(build(palette), uid));
    console.log("ok", `${uid}.svg`);
  }
}
console.log("done");
